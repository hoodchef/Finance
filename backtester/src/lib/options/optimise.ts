import { priceOption, yearsBetween } from './pricing';
import {
  capitalRequired,
  netDebit,
  profitAtExpiry,
  ...
  const premium = priceOption(
    {
      spot: req.spot,
      strike,
      timeToExpiry: yearsBetween(req.asOf, expiry),
...
function expectedValueUnderDrift(position: OptionPosition, req: OptimiseRequest): number {
  ...
  const T = yearsBetween(req.asOf, expiries[expiries.length - 1] ?? req.asOf);
...
/** Local inverse normal, so this module does not depend on the chart layer. */
